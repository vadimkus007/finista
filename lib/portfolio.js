    ...
    static getSecids(portfolioId, date) {
        if (typeof date !== 'undefined') {
            return Trade.findAll({
                attributes: [[sequelize.fn('DISTINCT', sequelize.col('secid')), 'secid'], 'group'],
                include: [
                    {
...
// console.log('PORTFOLIO.TRADES', trade);
    ...
    static getDividends(portfolioId, date) {
        if (typeof year !== 'undefined') {
            return Trade.findAll({
                    attributes: [
                        [sequelize.fn('sum', sequelize.literal('`Trade`.`price`*`Trade`.`amount` - `Trade`.`comission`')), 'dividends']
                    ],
                    where: {
                        [Op.and]: [
                            {'portfolioId' : portfolioId},
                            sequelize.literal('`Trade`.`operationId` IN (3, 5)'),
                            sequelize.where(sequelize.fn('date', sequelize.col('date')), '<=', date)
                        ]
                    },
                    raw: true
    ...
    static getDividendsYear(portfolioId, year) {
        return Trade.findAll({
                    attributes: [
                        [sequelize.fn('sum', sequelize.literal('`Trade`.`price`*`Trade`.`amount` - `Trade`.`comission`')), 'dividends']
                    ],
    ...
    // getCurrencies depending on portfolio.currency
    static getCurrencies(portfolioId) {
        return Trade.findAll({
            attributes: [
                'date',
                'operationId',
    ...
    */
    ...
    static getSecidsAmount(portfolioId, secids, date) {
        ...
        const findSecid = (array, secid) => {
            let obj = {};
            obj = array.find((element) => {
    ...
    static getSecidProfit(portfolioId, secid, price) {
        var _self = this;
        var data = {
            cost: 0,
            buy: 0,
        ...
        return new Promise((resolve, reject) => {
            ...
            _self.getSecidAmount(portfolioId, secid)
            .then(amount => {
                data.cost = Number(amount.amount) * Number(price);

                return _self.getTrades(portfolioId);
            })
            .then(trades => {
                ...
                trades.forEach(trade => {
                    if (trade.secid === secid) {
                        switch (trade.operationId) {
                            case 1:
                                data.buy += trade.amount * trade.price + Number(trade.comission);
                                break;
                            case 2:
                                data.sell += trade.amount * trade.price - Number(trade.comission);
                                break;
                            case 3:
                                data.dividends += Number(trade.price * trade.amount) - Number(trade.comission);
                                break;
                            case 4:
                                data.sell += trade.price * trade.amount - Number(trade.comission);
                                break;
                            case 5: 
                                data.dividends += Number(trade.price * trade.amount) - Number(trade.comission);
                                break;
                            case 6:
                                ...
                                break;
                            case 7: 
                                data.buy += trade.price*trade.amount*trade.value/100 + Number(trade.accint) + Number(trade.comission);
                                break;
                            case 8:
                                data.sell += trade.price*trade.amount*trade.value/100 - Number(trade.accint) - Number(trade.comission);
                                break;
                            default:
                                break;
                        }
                ...
                resolve({
                    secid: secid,
                    profit: Number(data.cost) + Number(data.sell) + Number(data.dividends) - Number(data.buy)
                });
